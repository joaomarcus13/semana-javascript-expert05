export default class AppController {
    ...
    async onProgress({ processedToAlread, filename }) {
        console.log({ processedToAlread, filename })

        const file = this.uploadingFiles.get(filename)
        const alreadyProcessed = Math.ceil((processedToAlread / file.size) * 100)
    ...
    async onFileChange(files) {
        ...
        this.viewManager.updateStatus(0)
        this.uploadingFiles.clear()//bug conhecido sempre vai iniciar do zero
